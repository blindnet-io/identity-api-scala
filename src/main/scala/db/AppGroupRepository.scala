...
class AppGroupRepository(xa: Transactor[IO]) {
  ...
  def insert(id: UUID, accId: UUID, name: String, key: String): IO[Unit] =
    sql"insert into application_groups (id, accid, name, key) values ($id, $accId, $name, $key)".update.run
      .transact(xa)
      .void

  def updateName(accId: UUID, id: UUID, name: String): IO[Unit] =
    sql"update application_groups set name=$name where accid=$accId and id=$id".update.run
      .transact(xa)
      .void

  def updateKey(accId: UUID, id: UUID, key: String): IO[Unit] =
    sql"update application_groups set key=$key where accid=$accId and id=$id".update.run
      .transact(xa)
      .void

  def delete(accId: UUID, id: UUID): IO[Unit] =
    sql"delete from application_groups where accid=$accId and id=$id".update.run.transact(xa).void
